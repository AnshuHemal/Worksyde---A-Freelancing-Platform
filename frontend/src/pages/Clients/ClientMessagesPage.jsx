...
import { FiDownloadCloud, FiSmile } from "react-icons/fi";
...
const ClientMessagesPage = () => {
  ...
  // Remove mockChats, add real chat list state
  ...
  // Add state to cache userId -> name
    ...
    axios
      ...
      .then((res) => {
        ...
        if (data.success && Array.isArray(data.rooms)) {
          ...
          // setSelectedChat(data.rooms[0] || null); // Removed auto-selection
        } else {
          setChatList([]);
        }
  ...
  // Fetch freelancer info for chat list
  useEffect(() => {
    ...
    chatList.forEach((chat, idx) => {
      if (
        ...
        (!chat.freelancer_name ||
          !chat.freelancer_avatar ||
          !chat.freelancer_title)
      ) {
        axios
          .get(`/api/auth/freelancer/profile/${chat.other_user_id}/`)
          .then((res) => {
            const hasPhoto = res.data && res.data.photograph === true;
            const avatarUrl = hasPhoto
              ? `/api/auth/profile-image/${chat.other_user_id}/`
              : null;
            setChatList((prev) => {
              const updated = [...prev];
              updated[idx] = {
                ...updated[idx],
                freelancer_name: res.data.name,
                freelancer_avatar: avatarUrl,
                freelancer_title: res.data.title,
              };
              return updated;
            });
          })
  ...
  useEffect(() => {
    if (!selectedChat || !selectedChat.other_user_id) return;
    // Only fetch if not already present
    if (!selectedChat.freelancer_name || !selectedChat.freelancer_title || !selectedChat.freelancer_avatar) {
      axios
        .get(`/api/auth/freelancer/profile/${selectedChat.other_user_id}/`)
        .then((res) => {
          const hasPhoto = res.data && res.data.photograph === true;
          const avatarUrl = hasPhoto
            ? `/api/auth/profile-image/${selectedChat.other_user_id}/`
            : null;
  ...
  useEffect(() => {
    ...
    messages.forEach((msg) => {
      if (
        msg.sender_id === selectedChat.other_user_id &&
  ...
  // When a chat is opened and messages are loaded, store the latest message timestamp in localStorage
  useEffect(() => {
    if (
      selectedChat &&
      ...
      if (lastMsg) {
        localStorage.setItem(
          `lastSeen_${selectedChat.room_id}_${currentUserId}`,
          lastMsg.timestamp
        );
      }
    }
            ...
            if (chat.room_id === data.room_id) {
              if (
                ...
                if (
                  ...
                  const senderName = chat.freelancer_name || "New message";
                  const senderAvatar = chat.freelancer_avatar;
                  toast.custom(
                    (t) => (
                      <div
                        style={{
  ...
  // Fetch sender names for filesAndLinks if not already cached
  useEffect(() => {
    ...
    uncachedIds.forEach((id) => {
      fetch(`/api/auth/freelancer/profile/${id}/`)
        .then((res) => res.json())
        .then((data) => {
          if (data && data.name) {
  ...
  const handleEmojiSelect = (emoji) => {
    // emoji is now an object with .native property
  ...
  // Real-time transliteration for Hindi and Gujarati using Google Input Tools API
  ...
  // Accept suggestion (on Tab or click)
  ...
  const filteredData = {
    ...
    emojis: Object.fromEntries(
      Object.entries(data.emojis).filter(([key, emoji]) => {
        // Remove emojis whose shortcodes are '0', '1', ..., '9' or whose unicode is 0-9 keycap
        ...
        // Remove keycap number emojis (e.g., 0️⃣, 1️⃣, ... 9️⃣)
        if (
          emoji.skins &&
          emoji.skins.some((skin) =>
            /\u003[0-9]\ufe0f\u20e3/.test(skin.unified)
          )
        )
          return false;
        return true;
  ...
  // Helper to get full date and time in IST
  ...
  // Group messages by date (array, not object)
  ...
  // Update filesAndLinks mapping to use sender's name and locale string for time
  const filesAndLinks = messages
    ...
    .map((msg) => {
      ...
      } else if (userNames[msg.sender_id]) {
        user = userNames[msg.sender_id];
      } else {
        user = msg.sender_id;
      }
      // Format date/time
          ...
          >
            ...
            {selectedChat ? (
              ...
              >
                <img
                  src={
                    selectedChat.freelancer_avatar ||
                    "https://randomuser.me/api/portraits/men/32.jpg"
                  }
                ...
                <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                  ...
                  >
                    {showFilesSidebar ? (
                      // Right arrow in circle SVG (hide)
                      <svg
                        width="22"
                        height="22"
                        viewBox="0 0 22 22"
